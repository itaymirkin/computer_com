...
#define WINDOW_SIZE 0 // Sending window size for checking for collisions (ms)

int running = 1;


int send_to_all_sockets(fd_set* sock_set, char* packet, int frame_size) {
    if (sock_set->fd_count > 1) {
...
int main(int argc, char* argv[]) {
    ...
    ULONGLONG select_window_start = 0;
    ...
    while (running) {
        ...
        ULONGLONG select_window_end = GetTickCount64(); ;
        
        if ((select_window_end - select_window_start) >= slot_time)
        {
            //printf("%%IM inside the SELECT %%\n");
            ...
            if (result == SOCKET_ERROR) {
                ...
                ULONGLONG select_window_start = GetTickCount64();
        ...
        //printf("end %ull\n start %ull \n", send_window_end, send_window_start);
        //printf("send window size %ull\n",send_window_end - send_window_start);

        if ((send_window_end - send_window_start) >= (ULONGLONG)WINDOW_SIZE) {
            server_is_sending = 0;
        ...
        for (int i = 1; i < sock_set.fd_count; i++) {
            ...
            if (FD_ISSET(current_sock, &read_fds)) {
                if (server_is_sending) {
                    conflict = 1;
                    char temp_buf[MAX_FRAME_SIZE];
                    int revc = recv(client_sock, temp_buf, sizeof(temp_buf), 0);
                    if (revc < 0) {
                        perror("recv");
                        ...
                        printf("Client %d is sending data while another client is sending\n", (int)client_sock);
                    }
        ...
        if (conflict) {
            ...
            packet[packet_size] = '\0'; // Null-terminate the string for printing
            printf("recived %s\n", &packet[HEADER_SIZE]);
